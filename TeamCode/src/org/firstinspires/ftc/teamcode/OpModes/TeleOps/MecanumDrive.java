...
@TeleOp (name ="testbot", group = "MecanumBot")
public class MecanumDrive extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        final Toggle northMode = new Toggle(false);
        ...
        while (opModeIsActive()) {
            ...
            double m = Math.atan2(y,x);
            northMode.toggle(gamepad1.a);


            if (northMode.getBool()){
               m = Math.atan2(y,x) - angle.firstAngle;
            }

            double z = 3 * Math.sqrt(x*x + y*y);

            double p1 = Math.sin(m + (double) 1/4*Math.PI ) * z;
            double p2 = Math.sin(m - (double) 1/4*Math.PI )* z;
            double p3 = Math.sin(m - (double) 1/4*Math.PI ) * z;
            double p4 = Math.sin(m + (double) 1/4*Math.PI)* z;
            ...
            telemetry.addData("North Mode", northMode.getBool());
